...
class ListSearchController {
    constructor(form) {
        this.form = form;
        this.input = form.querySelector("[data-list-search-input]");
        this.list = document.querySelector("[data-list-results]");
        this.status = document.querySelector("[data-list-search-status]");
        this.completions = form.querySelector("[data-list-search-completions]");
        this.label = form.dataset.listLabel;
    ...
    async updateResults() {
        ...
        pageUrl.searchParams.delete("resolved");
        pageUrl.searchParams.delete("rejected");
        const requestUrl = new URL(this.form.dataset.listUrl, window.location.origin);
        requestUrl.search = pageUrl.search;

        this.list.setAttribute("aria-busy", "true");
        this.status.textContent = `Updating ${this.label}…`;
        try {
            ...
            if (!response.ok) {
                throw new Error(`List search returned ${response.status}`);
            }

            const content = this.parseList(await response.text());
            this.list.replaceChildren(content);
            history.replaceState({}, "", pageUrl);
            this.status.textContent = `${this.label} updated.`;
        } catch (error) {
            if (error.name !== "AbortError") {
                this.status.textContent = `${this.label} could not be updated.`;
            }
        } finally {
    ...
    async loadCompletions() {
        ...
        const url = new URL(this.form.dataset.completionsUrl, window.location.origin);
...
function initializeListSearches() {
    for (const form of document.querySelectorAll("[data-list-search]")) {
        new ListSearchController(form).connect();
    }
}
